...
internal class PasswordApplicationService(
    ...
    fun delete(userId: Long, passwordId: Long, name: String, accessMode: AccessMode): Either<ErrorResponse, Long> {
        return transactionManager.executeWrite {
            return@executeWrite validateUserPermission(userFacade.findByUsername(name).orNull, userId)
                    .filterOrElse({ accessMode == AccessMode.MODIFY }) { ErrorResponse.readMode() }
                    .map { sharesFacade.deleteByPasswordId(passwordId) }
                    .map { passwordFacade.deleteByPasswordId(passwordId) }
        }
    }
